...
void Record_Delete(uint16 screen_id,uint16 control_id, uint16 line);
void Record_SetFocus(uint16 screen_id,uint16 control_id, uint16 line);
void Record_Update(uint16 screen_id,uint16 control_id, uint16 line, uint8 *record);
/*! 
 *  \brief     记录控件-设置记录显示偏移
 *  \param  screen_id 画面ID
 *  \param  control_id 控件ID
